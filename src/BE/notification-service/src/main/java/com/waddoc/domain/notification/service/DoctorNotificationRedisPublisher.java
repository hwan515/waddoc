...
public class DoctorNotificationRedisPublisher {
    ...
    public void publish(String doctorUserId, String eventName, NewBookingNotificationPayload payload) {
        DoctorNotificationBroadcastMessage message = new DoctorNotificationBroadcastMessage(
                doctorUserId, eventName, payload
        );
        notificationRedisTemplate.convertAndSend(doctorNotificationTopic.getTopic(), message);
        log.debug("Published doctor notification to Redis. doctorUserId={}, eventName={}", doctorUserId, eventName);
    }
}
