...
class DispatchConsumerTest {
    ...
    void consume_marksRetryPendingAndSendsDelaySmsWhenVehicleIsUnavailable() {
        ...
        assertThat(meterRegistry.get("waddoc.kafka.consumer.processed")
                ...
                .counter()
                .count()).isEqualTo(1.0);
        verify(businessEventOutboxService).enqueue(eq(EventTypes.DISPATCH_DELAYED_V1), eq("case_test123"), any(), any());
    ...
    void consume_createsMissionAndCompletesOutboxWhenVehicleIsOperational() {
        ...
        verify(businessEventOutboxService).enqueue(eq(EventTypes.DISPATCH_ASSIGNED_V1), any(), any(), any());
    }
    ...
    void consume_keepsOutboxPendingForOperatorDispatchWhenDemoModeIsEnabled() {
        ...
        assertThat(outbox.isRetryPending()).isTrue();
        verify(missionCommandService, never()).createMissionForDispatch(any(), any(), any(), any(), any());
        verify(businessEventOutboxService, never()).enqueue(eq(EventTypes.DISPATCH_DELAYED_V1), any(), any(), any());
    }
