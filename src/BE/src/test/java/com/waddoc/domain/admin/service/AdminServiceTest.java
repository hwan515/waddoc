...
class AdminServiceTest {
        ...
        void getCasesKeepsLatestStateWhenDuplicateMetadataExists() {
                ...
                when(careCaseRepository.findAllForAdmin(PageRequest.of(0, 20)))
                                .thenReturn(new PageImpl<>(List.of(careCase), PageRequest.of(0, 20), 1));
                when(missionRepository.findAllByCareCaseIn(List.of(careCase)))
                                .thenReturn(List.of(firstMission, secondMission));
                ...
                assertThat(response.getCases()).hasSize(1);
                assertThat(response.getCases().get(0).getMissionPhase()).isEqualTo(MissionPhase.VERIFYING);
                assertThat(response.getCases().get(0).getSessionStatus())
                                .isEqualTo(ConsultationSessionStatus.IN_PROGRESS);
        }
        ...
        void approveGuardianLinkRequestApprovesLinkAndGuardianUser() {
                ...
                when(patientGuardianLinkRepository.findDetailedByPublicId(link.getPublicId()))
                                .thenReturn(Optional.of(link));

                GuardianLinkApprovalResponse response = adminService.approveGuardianLinkRequest(admin,
                                link.getPublicId());
        ...
        void rejectGuardianLinkRequestRejectsAccountAndRevokesTokensWhenNoApprovedLinksRemain() {
                ...
                GuardianLinkRejectionResponse response = adminService.rejectGuardianLinkRequest(admin,
                                link.getPublicId());
