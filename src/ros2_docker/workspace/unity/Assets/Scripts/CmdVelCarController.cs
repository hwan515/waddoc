...
public class CmdVelCarController : MonoBehaviour
{
    ...
    public float maxMotorTorque = 12000f;        // 기본 최대 토크
    public float launchTorqueBoost = 1.35f;      // 출발 시 추가 토크
    public float maxSteerAngleLowSpeed = 52f;    // 저속 최대 조향각
    public float maxSteerAngleHighSpeed = 26f;   // 고속 최대 조향각
    public float steerResponse = 18f;            // 조향 반응 속도
    ...
    public float idleBrakeTorque = 0f;

    [Header("Timeout")]
    public float cmdTimeout = 0.6f;

    [Header("RigidBody")]
    public bool useCustomCenterOfMass = true;
    public Vector3 customCenterOfMass = new Vector3(0f, -0.35f, 0f);
    public float rbMass = 550f;
    public float rbLinearDamping = 0.04f;
    ...
    public float suspensionTargetPosition = 0.40f;
